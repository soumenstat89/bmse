...
for(model in 1:4){
  ...
  if(model == 2 | model == 4)
  {
    post.p0 = post[, 'p0']; post.logitp0 = logit(post.p0)
      ...
      # WAIC
      #=======================================
      waic.val = waicfn(loglik.chain.waic)
  
      out =  matrix(c(waic.val), 3, 2)
      
      dimnames(out) = list(c( 'WAIC1', 'WAIC2', 'WAIC3'), c('COL1', 'COL2'))
      
      write.csv(out, file = paste(folderpath, '/waic.csv', sep = ''), quote = F, row.names = T)
     
     
    } # end of for(model in 1:4)

#====================================================
save.image(paste0(folderpath, '/savingRimage_WAIC.RData', sep = ''))
